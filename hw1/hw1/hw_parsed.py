...
# Cell 12/54: markdown
"""
1.1 **Answer**: Each of the V words in the vocabulary is associated with two d-dimensional embedding vectors (w_i and w_tilde_i) and two scalar biases (b_i and b_tilde_i).

The total number of parameters is the sum of the sizes of all these parameters:
- Parameters for all w_i vectors: V * d
- Parameters for all w_tilde_i vectors: V * d
- Parameters for all b_i biases: V * 1
- Parameters for all b_tilde_i biases: V * 1

Total parameters = V*d + V*d + V + V = 2Vd + 2V = 2V(d+1).
"""
...
$L = \sum_{k,j=1}^V (\mathbf{w}_k^\top\tilde{\mathbf{w}}_j + b_k + \tilde{b}_j - \log X_{kj})^2$

To find the gradient with respect to $\mathbf{w}_i$, we only need to consider the terms in the sum where $k=i$:
$ \frac{\partial L}{\partial \mathbf{w}_i} = \frac{\partial}{\partial \mathbf{w}_i} \sum_{j=1}^V (\mathbf{w}_i^\top\tilde{\mathbf{w}}_j + b_i + \tilde{b}_j - \log X_{ij})^2 $
Using the chain rule:
$ \frac{\partial L}{\partial \mathbf{w}_i} = \sum_{j=1}^V 2 (\mathbf{w}_i^\top\tilde{\mathbf{w}}_j + b_i + \tilde{b}_j - \log X_{ij}) \frac{\partial}{\partial \mathbf{w}_i} (\mathbf{w}_i^\top\tilde{\mathbf{w}}_j) $
$ \frac{\partial L}{\partial \mathbf{w}_i} = 2 \sum_{j=1}^V (\mathbf{w}_i^\top\tilde{\mathbf{w}}_j + b_i + \tilde{b}_j - \log X_{ij}) \tilde{\mathbf{w}}_j $

Similarly, for the gradient with respect to the bias term $b_i$:
$ \frac{\partial L}{\partial b_i} = \frac{\partial}{\partial b_i} \sum_{j=1}^V (\mathbf{w}_i^\top\tilde{\mathbf{w}}_j + b_i + \tilde{b}_j - \log X_{ij})^2 $
$ \frac{\partial L}{\partial b_i} = \sum_{j=1}^V 2 (\mathbf{w}_i^\top\tilde{\mathbf{w}}_j + b_i + \tilde{b}_j - \log X_{ij}) \frac{\partial}{\partial b_i} (b_i) $
$ \frac{\partial L}{\partial b_i} = 2 \sum_{j=1}^V (\mathbf{w}_i^\top\tilde{\mathbf{w}}_j + b_i + \tilde{b}_j - \log X_{ij}) $
"""
...
2.  **Effect of `d` on validation error:**
    A larger `d` does not always lead to better validation error. This is due to the bias-variance tradeoff.
    - If `d` is too small, the model is too simple (high bias) and may underfit, failing to capture the structure of the data.
    - If `d` is too large, the model becomes too complex (high variance) and may overfit the training data, learning noise rather than the underlying signal. This hurts its ability to generalize to unseen data, leading to a higher validation error.
    The graphs show this pattern, where the validation error first decreases and then increases as `d` grows.

3.  **Which model is better?**
    The **asymmetric model performs better**, as it achieves a lower minimum validation loss (at d=10) than the symmetric model.
    The asymmetric model has more parameters and is more expressive. The co-occurrence data is inherently asymmetric (e.g., the probability of "York" following "New" is not the same as "New" following "York"). The asymmetric model can capture this, while the symmetric model is constrained (`W = W_tilde`), which limits its ability to model these asymmetric relationships.
"""
...
class Model(object):
    ...
    def compute_loss_derivative(self, output_activations, expanded_target_batch, target_mask):
        ...
        reshaped_mask = np.repeat(target_mask.reshape(batch_size, self.context_len), self.vocab_size, axis=1)
        # The derivative is (y - t) for the masked words, and 0 otherwise
        loss_derivative = reshaped_mask * (output_activations - expanded_target_batch)
    ...
    def sample_input_mask(self, batch_size):
        ...
        mask = np.zeros((batch_size, self.context_len), dtype=int)# Convert to one hot B x N, B batch size, N context len
